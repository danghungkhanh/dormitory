...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql_latest_reading = "SELECT electricity_kwh, month, year FROM utility_readings WHERE room_id = ? AND student_id = ? ORDER BY year DESC, month DESC LIMIT 1";
        ...
        $sql_check_paid = "SELECT COUNT(*) FROM payments WHERE user_id = ? AND room_id = ? AND payment_type = 'electricity' AND YEAR(payment_date) = ? AND MONTH(payment_date) = ? AND status = 'paid'";
